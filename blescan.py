        ...
        if event == bluez.EVT_INQUIRY_RESULT_WITH_RSSI:
            ...
            if subevent == EVT_LE_CONN_COMPLETE:
		    	...
		    	# commented out - don't know what this byte is.  It's NOT TXPower
                        txpower, = struct.unpack("b", pkt[report_pkt_offset -2].to_bytes(1, 'big'))
                        print ("\t(Unknown):", txpower)
                        rssi, = struct.unpack("b", pkt[report_pkt_offset -1].to_bytes(1, 'big'))
                        print ("\tRSSI:", rssi)
		    # build the return string
                    ...
                    Adstring += ","
                    Adstring += "%i" % struct.unpack("b", pkt[report_pkt_offset -2].to_bytes(1, 'big'))
                    Adstring += ","
                    Adstring += "%i" % struct.unpack("b", pkt[report_pkt_offset -1].to_bytes(1, 'big'))
